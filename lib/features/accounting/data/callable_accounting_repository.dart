...
class CallableAccountingRepository implements AccountingRepository {
  ...
  Stream<AccountingSummary> watchSummary({
    ...
    final salesSubscription = _salesPeriodQuery(
          businessId: businessId,
          from: periodStart,
          to: periodEnd,
        )
    ...
    final expensesSubscription = _expensesPeriodQuery(
          businessId: businessId,
          from: periodStart,
          to: periodEnd,
        )
  ...
  Stream<List<AccountingSale>> watchSales({
    ...
    return _salesPeriodQuery(businessId: businessId, from: from, to: to)
        .snapshots()
        .map((snapshot) {
          final sales = snapshot.docs
              .map(_saleFromDoc)
              .where((sale) {
                if (sale.paymentStatus == AccountingPaymentStatus.cancelled) {
                  return false;
                }

                return !sale.createdAt.isBefore(from) &&
  ...
  Stream<List<AccountingExpense>> watchExpenses({
    ...
    return _expensesPeriodQuery(businessId: businessId, from: from, to: to)
        .snapshots()
        .map((snapshot) {
  ...
  AccountingSummary _buildSummary({
    ...
    for (final doc in sales) {
      ...
      if (_text(data['paymentStatus']) ==
          AccountingPaymentStatus.cancelled.name) {
        continue;
      }
      ...
      if (saleType == AccountingSaleType.product.name) {
    ...
    for (final doc in expenses) {
      final data = doc.data();
      final expenseDate = _date(data['expenseDate']) ?? _date(data['createdAt']);
